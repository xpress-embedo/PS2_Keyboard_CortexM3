/**
 * @file	: lpc13xx_i2c.c
 * @brief	: Contains all functions support for I2C firmware library on LPC13xx
 * @version	: 1.0
 * @date	: 9. April. 2009
 * @author	: HieuNguyen
 *----------------------------------------------------------------------------
...
typedef struct
{
  ...
  void		(*inthandler)(LPC_I2C_TypeDef *i2c);   	/* Transmission interrupt handler */
} I2C_CFG_T;
...
static uint32_t I2C_Start (LPC_I2C_TypeDef *i2c);

/* Generate a stop condition on I2C bus (in master mode only) */
static void I2C_Stop (LPC_I2C_TypeDef *i2c);

/* I2C send byte subroutine */
static uint32_t I2C_SendByte (LPC_I2C_TypeDef *i2c, uint8_t databyte);

/* I2C get byte subroutine */
static uint32_t I2C_GetByte (LPC_I2C_TypeDef *i2c, uint8_t *retdat, Bool ack);

/* I2C interrupt master handler */
void I2C_MasterHandler (LPC_I2C_TypeDef *i2c);

/* I2C interrupt master handler */
void I2C_SlaveHandler (LPC_I2C_TypeDef *i2c);

/* Enable interrupt for I2C device */
void I2C_IntCmd (LPC_I2C_TypeDef *i2c, Bool NewState);
 ...
 * @param[in]	i2c	I2C peripheral selected, should be I2C0, I2C1 or I2C2
 ...
 * @return 		None
 ***********************************************************************/
void I2C_SetClock (LPC_I2C_TypeDef *i2c, uint32_t target_clock)
{
	uint32_t temp;
 ...
 * @return 		None
 **********************************************************************/
void I2C_DeInit(LPC_I2C_TypeDef* i2c)
{
 ...
 *********************************************************************/
 #if 1
void I2C_Init(LPC_I2C_TypeDef *i2c, uint32_t clockrate)
{
 ...
 * @return 		none
 **********************************************************************/
void I2C_Cmd(LPC_I2C_TypeDef* i2c, FunctionalState NewState)
{
 ...
 * @param[in] 	i2c				Pointr to a LPC_I2C_TypeDef structure
 ...
 * corresponding.
 **********************************************************************/
Status I2C_MasterTransferData(LPC_I2C_TypeDef *i2c, I2C_M_SETUP_Type *TransferCfg, \
		...
		if ((TransferCfg->rx_length != 0) && (TransferCfg->rx_data != NULL)){
			...
			while (TransferCfg->rx_count < TransferCfg->rx_length){
				...
				if (TransferCfg->rx_count < (TransferCfg->rx_length - 1)){
					...
					CodeStatus = I2C_GetByte(i2c, &tmp, TRUE);
					...
					CodeStatus = I2C_GetByte(i2c, &tmp, FALSE);
 ...
 * @param[in]	i2c				Pointer to a LPC_I2C_TypeDef structure
 ...
 **********************************************************************/
Status I2C_SlaveTransferData(LPC_I2C_TypeDef *i2c, I2C_S_SETUP_Type *TransferCfg, \
								I2C_TRANSFER_OPT_Type Opt)
{
	uint8_t *txdat;
	uint8_t *rxdat;
 ...
 * @return 		None
 **********************************************************************/
void I2C_SetOwnSlaveAddr(LPC_I2C_TypeDef *i2c, I2C_OWNSLAVEADDR_CFG_Type *OwnSlaveAddrConfigStruct)
{
	uint32_t tmp;
 ...
 * @return		None
 **********************************************************************/
void I2C_MonitorModeConfig(LPC_I2C_TypeDef *i2c, uint32_t MonitorCfgType, FunctionalState NewState)
{
 ...
 * @return		None
 **********************************************************************/
void I2C_MonitorModeCmd(LPC_I2C_TypeDef *i2c, FunctionalState NewState)
{
 ...
 * new data.
 **********************************************************************/
uint8_t I2C_MonitorGetDatabuffer(LPC_I2C_TypeDef *i2c)
{
